...
import { GET_POSTS_BY_OWNER, GET_ALL_TAGS } from "../operations/queries";
  ...
  return (
    <>
      <Row className="align-items-center mb-4">
        <Col>
          <h1>See what has been posted</h1>
        </Col>
        <Col xs="auto">
...
function NoteCard({ id, title, tags, item_name, filename }: SimplifiedNote) {
  const imageUrl = `https://beachfinds-uploadserver.azurewebsites.net/api/v1/upload/${filename}`;
  return (
    <Card
      as={Link}
      to={`/${id}`}
      ...
      <Card.Body>
        ...
        >
          ...
          {tags.length > 0 && (
            ...
            >
              {tags.map((tag) => (
                <Badge className="text-trunccate" key={tag.id}>
                  {tag.label}
                </Badge>
              ))}
            </Stack>
